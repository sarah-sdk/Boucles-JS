...
const hiddenCodeString = hiddenCode.split('');

for (let i = 0; i < hiddenCodeString.length; i++){
    if (hiddenCodeString[i] === 'X'){
        hiddenCodeString.splice([i], 1);
        i--;
    }
    else if (parseInt(hiddenCodeString[i]) % 2 == 0 ){
        hiddenCodeString.splice([i], 1);
        i--;
    }
...
console.log(hiddenCodeString.join(''));
